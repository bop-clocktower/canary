...
function collectConsent(
  ...
  out.write(
    `Overlay "${name}" ships ${cmds.length} command check(s) that 'canary doctor' can run:\n`,
  );
  for (const c of cmds) {
    out.write(`  - ${c.id}: ${(c.command ?? []).join(' ')}\n`);
  }
  const granted = confirm(
    `Allow 'canary doctor' to run these commands for "${name}"? [y/N] `,
  );
  if (!granted) {
    out.write(
...
const CLONE_FAILURE_SIGNATURES: ReadonlyArray<{
  reason: string;
...
export function add(
  ...
  const { consent, consentCommandsHash } = collectConsent(
    dest,
    parsed.name,
    confirm,
    out,
  ...
  out.write(
    `Added overlay "${parsed.name}"${ref ? ` @ ${ref}` : ''} → ${dest}\n`,
  );
  return 0;
}
...
export function list(deps: CommandDeps = {}, opts: ListOptions = {}): number {
  ...
  if (reg.overlays.length === 0) {
    out.write(
      "No overlays added. Add one with 'canary overlay add <source>'.\n",
    );
    return 0;
  }
...
function updateOne(
  ...
  if (!fs.existsSync(o.path)) {
    err.write(
      `overlay "${o.name}": clone missing at ${o.path} — remove and re-add it.\n`,
    );
    return 1;
  }
  ...
  if (o.ref) {
    ...
    if (co.status !== 0) {
      err.write(
        `overlay "${o.name}": checkout ${o.ref} failed.\n${co.stderr.trim()}\n`,
      );
      return 1;
    }
