...
import { freshness, workingTreeStatus, type GitRunner, type GitResult } from "./overlay-commands.js";
...
const realGit: GitRunner = (args, opts = {}) => {
  ...
  return { status: r.status ?? 1, stdout: r.stdout ?? "", stderr: r.stderr ?? "" } as GitResult;
};
...
function ownVersion(): string | null {
  try {
    ...
    return (JSON.parse(fs.readFileSync(pkgPath, "utf8")) as { version?: string }).version ?? null;
  } catch {
    return null;
  }
}
...
export async function checkVersion(deps: EngineCheckDeps = {}): Promise<CheckResult> {
...
export async function runEngineChecks(deps: EngineCheckDeps = {}): Promise<CheckResult[]> {
  return [
