...
describe('parseSource — github: shorthand', () => {
  const cases = [
    [
      'github:example-org/example-overlay',
      'https://github.com/example-org/example-overlay.git',
      'example-org-example-overlay',
    ],
    [
      'github:example-org/example-overlay.git',
      'https://github.com/example-org/example-overlay.git',
      'example-org-example-overlay',
    ],
  ];
...
describe('parseSource — full URLs', () => {
  it('https URL passthrough, name from last two segments', () => {
    assert.deepEqual(
      parseSource('https://github.com/example-org/example-overlay.git'),
      {
  ...
  it('scp-style git@ URL passthrough', () => {
    assert.deepEqual(
      parseSource('git@github.com:example-org/example-overlay.git'),
      {
  ...
  it('ssh:// URL', () => {
    assert.deepEqual(
      parseSource('ssh://git@example.com/example-org/example-overlay'),
      {
