...
function makeSourceRepo(manifest) {
  ...
  if (manifest !== null) {
    fs.writeFileSync(
      path.join(work, '.canary', 'doctor.json'),
...
describe('overlay add — consent gate', () => {
  ...
  it('records consent:null when the overlay ships no command checks', () => {
    src = makeSourceRepo({
      checks: [
        { id: 'f', type: 'file-exists', path: 'README.md', remedy: 'r' },
      ],
    });
    let prompted = false;
    assert.equal(
      addFrom(src, () => ((prompted = true), true)),
      0,
    );
    const entry = registry.read(home).overlays[0];
    ...
    assert.equal(
      prompted,
      false,
      'must not prompt when there are no command checks',
    );
  });
...
describe('consentGranted predicate', () => {
  ...
  it('is false when consent was declined', () => {
    assert.equal(
      registry.consentGranted(
        { consent: false, consentCommandsHash: hash },
        hash,
      ),
      false,
    );
