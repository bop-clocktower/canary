...
describe("overlay update", () => {
  it("fast-forwards when the remote advanced", () => {
    ...
    assert.ok(fs.existsSync(path.join(clone, "NEW.md")), "clone should have the new commit");
  });
  ...
  it("refuses when the clone has local modifications", () => {
    ...
    assert.match(err.get(), new RegExp(clone.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  ...
  it("errors on an unknown overlay name", () => {
    const err = collector();
    const code = cmd.update("no-such-overlay", { homeDir: home, out: collector(), err });
    assert.equal(code, 1);
