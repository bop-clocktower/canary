...
describe("overlay add", () => {
  it("clones a local source and registers it", () => {
    const out = collector();
    const code = cmd.add(source, {}, { homeDir: home, out, err: collector(), now: () => "2026-07-16" });
    assert.equal(code, 0);
    const name = path.basename(source);
    const dest = registry.clonePath(name, home);
    assert.ok(fs.existsSync(path.join(dest, ".canary", "skills", "demo", "SKILL.md")));
  ...
  it("is idempotent — re-adding is a no-op with an update hint", () => {
    cmd.add(source, {}, { homeDir: home, out: collector(), err: collector(), now: () => "2026-07-16" });
    const out = collector();
  ...
  it("leaves nothing registered and no partial clone when the clone fails", () => {
    ...
    assert.equal(fs.existsSync(registry.clonePath("canary-does-not-exist-xyz", home)), false);
    assert.match(err.get(), /clone failed/);
  });

  it("rejects a malformed source spec", () => {
    const err = collector();
    const code = cmd.add("not-a-source", {}, { homeDir: home, out: collector(), err });
    assert.equal(code, 1);
...
describe("classifyCloneFailure", () => {
  it("maps common git stderr to a reason", () => {
    assert.equal(cmd.classifyCloneFailure({ status: 128, stdout: "", stderr: "fatal: could not resolve host: github.com" }), "network unreachable");
    assert.equal(cmd.classifyCloneFailure({ status: 128, stdout: "", stderr: "fatal: Authentication failed for 'https://...'" }), "authentication denied");
    assert.equal(cmd.classifyCloneFailure({ status: 128, stdout: "", stderr: "fatal: repository not found" }), "repository not found");
    assert.equal(cmd.classifyCloneFailure({ status: 127, stdout: "", stderr: "spawn git ENOENT" }), "git not found on PATH");
  });
});
