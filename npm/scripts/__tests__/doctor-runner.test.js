...
let clone;
beforeEach(
  () => (clone = fs.mkdtempSync(path.join(os.tmpdir(), 'canary-run-'))),
);
...
describe('runCheck: file-exists', () => {
  it('passes when the file exists (relative to clone)', async () => {
    ...
    const r = await runCheck(
      { id: 'f', type: 'file-exists', path: 'present.txt', remedy: 'add it' },
      ctx(),
    );
  ...
  it('fails with the manifest remedy when missing', async () => {
    const r = await runCheck(
      { id: 'f', type: 'file-exists', path: 'nope.txt', remedy: 'add it' },
      ctx(),
    );
...
describe('runCheck: command-succeeds', () => {
  ...
  it('is SKIPPED (not failed) without consent, and never runs the command', async () => {
    ...
    const r = await runCheck(
      ...
      ctx({
        consentGranted: false,
        runCommand: () => ((ran = true), { ok: true, timedOut: false }),
      }),
    );
  ...
  it('passes when the command exits 0 (with consent)', async () => {
    const r = await runCheck(
      ...
      ctx({
        consentGranted: true,
        runCommand: () => ({ ok: true, timedOut: false }),
      }),
    );
    assert.equal(r.status, 'pass');
  });
  it('fails on non-zero exit', async () => {
    const r = await runCheck(
      ...
      ctx({
        consentGranted: true,
        runCommand: () => ({ ok: false, timedOut: false, detail: 'exit 1' }),
      }),
    );
    assert.equal(r.status, 'fail');
  ...
  it('fails with a timeout label when the command is killed', async () => {
    const r = await runCheck(
      ...
      ctx({
        consentGranted: true,
        timeoutMs: 50,
        runCommand: () => ({ ok: false, timedOut: true }),
      }),
    );
    assert.equal(r.status, 'fail');
