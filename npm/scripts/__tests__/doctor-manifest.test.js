...
function writeManifest(dir, obj) {
  fs.writeFileSync(path.join(dir, ".canary", "doctor.json"), typeof obj === "string" ? obj : JSON.stringify(obj));
}
...
describe("loadManifest", () => {
  ...
  it("parses a valid manifest across all three types", () => {
    writeManifest(clone, {
      checks: [
        { id: "cfg", type: "file-exists", path: ".canary/company.json", remedy: "re-clone" },
        { id: "reachable", type: "url-reachable", url: "https://example.com", persona: ["alpha"], remedy: "req access" },
        { id: "build", type: "command-succeeds", command: ["npm", "run", "x"], remedy: "install deps" },
      ],
    });
    const r = loadManifest(clone);
  ...
  it("rejects an unknown check type as a single failing check", () => {
    writeManifest(clone, { checks: [{ id: "x", type: "port-open", remedy: "y" }] });
    const r = loadManifest(clone);
    assert.equal(r.ok, false);
  ...
  it("rejects a file-exists check missing its path", () => {
    writeManifest(clone, { checks: [{ id: "x", type: "file-exists", remedy: "y" }] });
    const r = loadManifest(clone);
    assert.equal(r.ok, false);
  ...
  it("rejects a check missing its remedy", () => {
    writeManifest(clone, { checks: [{ id: "x", type: "url-reachable", url: "https://e.com" }] });
    const r = loadManifest(clone);
    assert.equal(r.ok, false);
...
describe("filterByPersona", () => {
  const checks = [
    ...
    { id: "alpha-only", type: "file-exists", path: "b", remedy: "r", persona: ["alpha"] },
    { id: "beta-only", type: "file-exists", path: "c", remedy: "r", persona: ["Beta"] },
  ];
