...
| D2  | Hybrid mechanic: engine helper consumed by ts commands + npm scripts; convention + table-driven conformance suite for self-contained skill CLIs   | Only skill CLIs need convention, and the conformance table holds them to it. **Amended 2026-08-03 (Wave 3):** npm cannot _import_ `dist/engine/` — npm is CommonJS, the staged bundle is ESM, and `npm test` never builds it. Since `gate-result.ts` has zero imports, npm consumes it as a drift-checked generated copy (`npm/scripts/sync-gate-result.mjs`, verified as `pretest`) — same single source of truth, no runtime bridge |
...
- v6.4.0. CHANGELOG gains a "Gates that got louder" section enumerating every
  surface whose exit code can newly be 3 and the one-line reason. **Amended
  2026-08-03:** v6.4.0 shipped Waves 1-2 under ordinary Added/Fixed headings, so
  the section does not exist yet. Decision: keep the requirement and land it in
  **Wave 5**, backfilling the v6.4.0 surfaces (`migrate --check`,
  `guardian pr-check`, `guardian harden-gate --apply`) next to Wave 3's `doctor`
  — one complete table beats three partial ones spread across releases. Success
  Criterion 6 is satisfied at the Wave 5 boundary, not per wave.
