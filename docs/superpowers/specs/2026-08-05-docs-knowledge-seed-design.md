...
**Date:** 2026-08-05 **Status:** implemented 2026-08-10 **Related:** PR #563
(false-green class), issue #564 (domainBlocklist invariant), PR #555, PR #561

## Implementation record (2026-08-10)

Implemented as designed. Four deviations, all discovered during verification:

1. **12 ADRs, not 10.** 0011 (required status checks) and 0012 (entropy ratchet)
   landed after this spec was written.
2. **22 files carried `docs/adr` references, not 21.** The extra is
   `.harness/entropy-baseline.json`, added with ADR 0012.
3. **One ADR was silently rejected by the ingestor.** Prettier wrapped ADR
   0007's long `title:` onto a continuation line — valid YAML, but harness
   parses frontmatter with `/^(\w+):\s*(.+)$/`, so a bare `title:` matched
   nothing and the file returned `null`. Verification gate 1 caught it as
   `11 decisions` against 12 files; nothing else would have. The title was
   shortened, and `ts/test/adr-ingestable.test.ts` now ports harness's parser
   exactly so the class cannot recur silently.
4. **Each ADR carries `<!-- markdownlint-disable-file MD025 -->`.** markdownlint
   reads a frontmatter `title:` as a top-level heading, making the real
   `# ADR NNNN` a duplicate H1. `disable-next-line` was tried first and does not
   survive prettier, which inserts a blank line between the directive and the
   heading. Weakening `.markdownlint.json` instead is blocked by
   `protect-config.js`, correctly.

Both upstream defects this work depended on are now filed:
[harness-engineering#1330](https://github.com/Intense-Visions/harness-engineering/issues/1330)
(hardcoded `decisionsDir`) and
[#1331](https://github.com/Intense-Visions/harness-engineering/issues/1331)
(extractors emitting keywords and config fragments as business knowledge).
