...
| `3`  | **Abstained** — nothing was verified, either because no overlay could be resolved at all (track one with `canary overlay add`, or pass `--from <overlay>`) or because the resolved shape set matched zero overlay skills (fix shape resolution — `canary_shape` in `.canary/company.json`, or `--framework` — or the overlay's `deploy_to` lists). A gate that checked nothing has not passed. |
