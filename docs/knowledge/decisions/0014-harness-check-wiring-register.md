...
| `scan-config`             | **Re-measured 2026-09-04 at CLI 12.2.0: 32 findings, all the same false-positive class.** The `fileGlob` fix (harness-engineering#1344) HAS reached the pin — `SEC-AGT-007` and `SEC-MCP-002` are both **0**, down from 392 and 391 — so half this row's revisit condition is met. The residual is not: 31 `INJ-SUS-001`/`INJ-SUS-002` matching the pipe characters of ordinary Markdown TABLES in `AGENTS.md`, plus 1 `SEC-PTH-001` (medium) matching a `readFileSync` one-liner quoted as a code SAMPLE in the same file. Identical root class — documentation prose read as an executable config directive — at a new rule id and a lower severity. Still unwireable, and now for a second reason: `scan-config` offers no severity threshold and no ignore mechanism, and its only other flag is `--fix`, which strips matched patterns from files **in place** and would silently edit documentation. Tracked in #719. | Prose is excluded from the `INJ-SUS-*`/`SEC-PTH-*` denominator upstream, or the command grows a severity threshold |
