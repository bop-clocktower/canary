...
The second command exists because the first was documented as sufficient and
was not. `strict` is set on the ruleset, never applied from the manifest, so
for that field the manifest is an observation rather than a declaration — and
it recorded `false` against a live `true` for three weeks (2026-08-12 to
2026-09-03) while `.rules[].type` kept reporting exactly what it was asked to.
A check that only asks whether a rule _exists_ cannot notice that its
parameters changed underneath.
...
part of landing any queued PR. No `merge_queue` rule is configured, which is
the feature that would remove that step.
