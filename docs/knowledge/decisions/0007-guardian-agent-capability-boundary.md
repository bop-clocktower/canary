---
number: 7
title: 'ADR 0007 — Guardian agent capability boundary'
date: 2026-07-19
status: accepted
source: adr
---
...
must-have value is a ranked, fidelity-labeled findings comment on **every** pull
request.

The complication is runtime. The high-value tiers (LLM test-quality audit, LLM
test authoring) need a Claude-compatible agent runtime. This repo has no
...
At the same time, an agent runtime _does_ exist where developers work — the
...
- **Tiers 1/2 engage only at the desk in v1.** The LLM test-quality audit (Tier
  1, via `canary-review-test`) and test authoring (Tier 2/desk, via
...
- **The two halves meet at an `AgentTier` capability interface**
  (`agent/guardian/agent_tier.py`). `InSessionAgentTier` implements it for v1; a
  future `CiAgentTier` (a runner) can implement the same protocol to host the
...
- A future CI-agent runner attaches by implementing `AgentTier` (`CiAgentTier`)
  ...
  finding shape. The seam was shaped _after_ the deterministic surfaces shipped
...
- **Desk-only Tiers 1/2 mean CI never audits test _quality_ in v1** — only
...
fork/concurrency loop-guards — _before_ the deterministic baseline has earned
...
_guaranteed_ baseline to the LLM: every CI run would then need an agent runtime,
