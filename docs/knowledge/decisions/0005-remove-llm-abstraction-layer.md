---
number: 5
title: 'ADR 0005 — Remove LLM Abstraction Layer (v3.0)'
date: 2026-05-28
status: accepted
source: adr
---
...
**Status:** accepted **Date:** 2026-05-28 **Deciders:** Bri Stevenski (upstream
maintainer) **Supersedes:** ADR 0003 (deprecation phase — removal now complete)
...
shrinks by ~50 MB (no provider SDKs). 48 tests removed; 460 remaining at time of
removal (suite has grown substantially since), all covering non-LLM commands.
...
`oracle generate --recommend-only`. Same classifier + recommender output, now a
first-class command rather than a flag on a deprecated command.

**Company knowledge injection:** the `--- COMPANY KNOWLEDGE ---` block that
`CanaryOrchestrator` appended to generation prompts is now the responsibility of
the slash command layer. The `/oracle-write-test` command invokes
