...
function inlineScan(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // HIGH: INJ-ENC-001 — Suspicious base64. Gated on entropy: the run must contain
    // a digit AND a lowercase AND an uppercase char. Real base64 of binary data
    // mixes all three; slash-delimited filesystem paths and URLs (the prior
    // false-positive source, e.g. "Users/bs/Github/canary/docs/roadmap") are
    // dictionary segments that lack digits, so they no longer match. Genuine
    // 4-aligned payloads (≥28 chars, padded or boundary-terminated) still trip,
    // including ones that contain '/'.
    if (!/^[\s]*(?:cd |git |node |pnpm |npm |\/|packages\/|agents\/|src\/)/.test(line) &&
        /(?<!Bearer\s)(?<![:])(?<![A-Za-z0-9/])(?!eyJ)(?=[A-Za-z0-9+/]*[0-9])(?=[A-Za-z0-9+/]*[a-z])(?=[A-Za-z0-9+/]*[A-Z])(?:[A-Za-z0-9+/]{4}){7,}(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?(?![A-Za-z0-9/])/.test(line)) {
