...
![version](https://img.shields.io/badge/version-8.0.0-F0C040?style=flat-square&labelColor=0A0A0A&color=F0C040)
...
| `canary history record <vitest.json\|playwright.json\|junit.xml> --suite <name>`              | CLI — record a finished run INTO the store (the write side), with run and test durations.                                                                                             |
...
`flake-check`, `heal-test`, `vacuity-check`, `promote-check`, `ci-ready`) are
fully deterministic and make no LLM calls; they're documented as such above and
in each command's own `--help`.
