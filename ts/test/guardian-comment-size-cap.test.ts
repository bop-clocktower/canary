 ...
 *
 * `renderFindings(..., 'comment')` emitted every finding in full with no cap.
 * GitHub rejects an issue/PR comment body over **65,536 characters**, and the
 * guardian's post path treats that failure as "could not post" — so on a large
 * PR the gate would silently produce nothing — the silent-green failure mode
...
describe('comment size cap (#457)', () => {
  it('leaves a small comment untouched', () => {
    const body = renderFindings(many(3), 'comment');
  ...
  it('keeps a huge comment under the budget', () => {
    ...
    const body = renderFindings(many(2000, Severity.HIGH, 300), 'comment');
  ...
  it('truncates the LEAST severe first', () => {
    ...
    const body = renderFindings([...flood, critical], 'comment');
  ...
  it('NEVER truncates the json record', () => {
    ...
    const payload = JSON.parse(renderFindings(findings, 'json'));
