...
export async function invokeGuardian(
  ...
  // override. Snapshotting only GUARDIAN_ENV_KEYS let an off-list override
  // leak into process.env for every later test in the worker -- found by
  // canary-savant (SV003) once #495 gave it the precision to be heard.
  const savedEnv: Record<string, string | undefined> = {};
  for (const k of [...GUARDIAN_ENV_KEYS, ...Object.keys(env)]) {
