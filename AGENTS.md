...
##### Branch cleanup (ADR 0022)

`harness-repo-hygiene` is the branch cleanup path; stop hand-pruning with
`git branch -d` in a loop. Its deletion set is verified by
`node scripts/branch-prune.mjs` (dry-run by default), which classifies with
`git cherry` plus `git ls-remote`, never `git status`, and never offers a
candidate that is on the permanent protect-list (`main`, `release/*`,
`fix/mcp-tool-path-containment`, `fix/migrator-workflow-symlink-escape`) or
checked out in any worktree. `--apply-local` deletes local candidates only;
remote deletion is a human act. Worktree pruning waits for #889.
...
### Mutation testing (advisory, #486)

`canary guardian mutation` reports which mutated lines a test covers but would
not fail on. A mutant nothing notices is a _surviving mutant_, and it is
concrete evidence of a weak assertion, which is what coverage cannot see.

The report covers whatever the Stryker run it maps covered; `--report` is not
narrowed to the diff, so it is not described as diff-scoped.

- **Advisory, on purpose.** The `mutation` job in `guardian.yml` never fails a
  PR on survivors, and it is listed under `advisory` in
  `.github/required-checks.json`, never under the required contexts. It is
  promoted to a gate only when the exit criterion in the spec (D5) is met on
  measured evidence.
...
- **A timeout is not a kill.** Timeouts are excluded from the killed numerator,
  so a run that killed nothing and only timed out abstains rather than reporting
  `all-killed`; every non-abstained report renders its timeout count, zero
  included.
...
- A row links to a ticket through `- **External-ID:** github:owner/repo#N`, an
  ...
  conclude no link field exists. All 28 rows carry one — 47 as of #628, plus the
  ...
  2026-09-14 `roadmap-groom --check-issues` reconciliation — and `Priority`,
  serialized in that same extended group, is populated on every one of them.
- `tracker.labels` in `harness.config.json` filters sync to `harness-managed`.
  ...
  effectively blind gate that reported a real number nobody read. All 28 rows
