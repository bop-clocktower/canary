...
about how to **report back** when Canary does something unexpected.

## Why this example exists

The fastest way to make Canary better is for testers to file specific issues
when output is wrong, surprising, or unhelpful. Canary ships with a one-click
...
Any `canary generate` works. Even the recommend-only path emits a hint.

```bash
canary generate "smoke test the homepage"
```

The last line of the output:

```text
💬 Report feedback (public link — review before submitting): https://github.com/bop-clocktower/canary/issues/new?title=...&body=...
```
...
> `data-testid="login-cta"`. Canary should default to data-testid selectors
...
This reads `.canary/last_generation.json` (gitignored, local-only) and reprints
...
```bash
export CANARY_FEEDBACK_REPO=your-org/private-canary-feedback
canary generate "anything"   # URLs now point at the private repo
```
...
```bash
canary generate "test something" --json | jq .feedback_url
```
...
- [CLI Reference → `canary feedback`](../../docs/wiki/CLI-Reference.md)
